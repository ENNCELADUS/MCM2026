...
Usage:
    ...
    python main.py --horizon 60             # 5-year horizon
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python main.py --scenario Mix --horizon 120
  python main.py --scenario E-only --output results/elevator_scenario
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="Planning horizon in months (default: 120 = 10 years)",
    )
...
def run_pipeline(
    ...
    if verbose:
        ...
        print(f"Horizon: {settings.T_horizon} months ({settings.T_horizon // 12} years)")
        print(f"Learning Curve: {'Enabled' if settings.enable_learning_curve else 'Disabled'}")
        print(f"Pre-positioning: {'Enabled' if settings.enable_preposition else 'Disabled'}")
    ...
    # ---------------------------------------------------------------------
    if verbose:
        print(f"[Step 4] Solving (timeout: {settings.solver_timeout}s, gap: {settings.mip_gap})...")
...
def main() -> int:
    ...
    constants_path = Path(args.constants) if args.constants else Path(__file__).parent / "config" / "constants.yaml"
