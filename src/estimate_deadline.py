...
def find_deadline(
    target: float, start_year: int = 2050, max_years: int = 200
) -> tuple[int, float]:
    cumulative = 0.0
...
if __name__ == "__main__":
    ...
    print(
        f"Target Mass: {TARGET_MASS_TONS:,.0f} tons ({TARGET_MASS_TONS / 1e6:.0f} Mt)"
    )
    print()
    ...
    for year in [2050, 2060, 2070, 2080, 2100]:
        ...
        print(
            f"{year}: Elevator {e_cap:,.0f} t/y | Rocket {r_cap:,.0f} t/y | Total {e_cap + r_cap:,.0f} t/y"
        )
