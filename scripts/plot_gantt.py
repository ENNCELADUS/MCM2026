...
def plot_gantt(phases: list[dict], output_path: Path, scenario: str) -> None:
    ...
    for i, phase in enumerate(valid_phases):
        ...
        ax.text(
            end + 0.3, i, f"{end:.0f}", ha="left", va="center", fontsize=9, color="gray"
        )
    ...
    for decade in range(2050, 2110, 10):
        if min_year < decade < max_year:
            ax.axvline(
                x=decade, color="lightgray", linestyle="--", linewidth=0.8, alpha=0.7
            )
    ...
    handles = [
        ...
        if any(p["phase"] == l for p in valid_phases)
    ]
    if handles:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Gantt Chart for Construction Phases"
    )
