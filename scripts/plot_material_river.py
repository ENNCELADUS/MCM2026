...
def plot_river(rows: list[dict], output_path: Path, scenario: str) -> None:
    ...
    earth = np.array([r["earth_arrivals_tons"] for r in rows]) / 1e6  # Convert to million tons
    ...
    earth_share = np.where(cumulative_total > 0, cumulative_earth / cumulative_total * 100, 100)
    isru_share = np.where(cumulative_total > 0, cumulative_isru / cumulative_total * 100, 0)
    ...
    ax1.fill_between(years, 0, cumulative_earth, label="Earth Supply", 
                     color=earth_color, alpha=0.8)
    ax1.fill_between(years, cumulative_earth, cumulative_earth + cumulative_isru, 
                     label="Lunar ISRU", color=isru_color, alpha=0.8)
    ...
    if cumulative_total.max() >= 100:
        ...
        ax1.annotate("100M Ton Target", xy=(years[len(years)//4], 102), 
    ...
    ax2.annotate(f"Final: {final_isru:.1f}%", 
                 xy=(years[-1], final_isru),
                 xytext=(-60, 10), textcoords="offset points",
                 fontsize=10, fontweight="bold", color=isru_color,
    ...
    title = f"Material Composition River Plot — {scenario_labels.get(scenario, scenario)}"
...
def main():
    parser = argparse.ArgumentParser(description="Generate Material Composition River Plot")
