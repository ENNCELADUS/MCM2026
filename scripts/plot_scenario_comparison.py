...
def plot_scenario_comparison(data: dict, output_path: Path) -> None:
    ...
    # Bar plots
    bars1 = ax1.bar(x - width/2, durations, width, label="Duration (years)", 
                    color=duration_color, alpha=0.85, edgecolor="white", linewidth=1.5)
    bars2 = ax2.bar(x + width/2, costs, width, label="Cost ($ Billions)", 
                    color=cost_color, alpha=0.85, edgecolor="white", linewidth=1.5)
    ...
    ax1.annotate("50-year Target", xy=(0.02, 51), fontsize=9, color="gray", style="italic")
    ...
    # Title
    fig.suptitle("Comparative Performance: Duration vs Cost by Scenario", 
