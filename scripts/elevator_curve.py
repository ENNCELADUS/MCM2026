...
axs[0, 0].plot(years_short, cap_short / 1e6, 'b-', linewidth=2, label='Logistic Growth Model')
axs[0, 0].axhline(y=C_E_ref / 1e6, color='red', linestyle='--', alpha=0.6, label=f'Initial Capacity ({C_E_ref/1e6:.3f} Mt/y)')
...
cost_short = exponential_cost_decay(years_short, C_start_cost, C_min_cost, lambda_annual, t0_cost)
axs[0, 1].plot(years_short, cost_short, 'g-', linewidth=2, label='Exponential Decay Model')
axs[0, 1].axhline(y=C_start_cost, color='red', linestyle='--', alpha=0.6, label=f'Initial Cost (${C_start_cost}/kg)')
axs[0, 1].axhline(y=C_min_cost, color='black', linestyle='--', alpha=0.6, label=f'Cost Floor (${C_min_cost}/kg)')
...
axs[1, 0].axhline(y=C_E_max / 1e6, color='black', linestyle='--', alpha=0.6, label=f'Physical Ceiling ({C_E_max/1e6:.1f} Mt/y)')
...
axs[1, 1].axhline(y=C_min_cost, color='black', linestyle='--', alpha=0.6, label=f'Cost Floor (${C_min_cost}/kg)')
