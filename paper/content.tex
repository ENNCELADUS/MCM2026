...
\subsection{The Environmental Investment Tipping Point}

\subsubsection{Coupled Seeding-Growth Dynamics}
The minimization of $\mathcal{D}_E$ is not achieved by simply eliminating rocket launches, but by leveraging them to accelerate the "Decoupling Point." We model the lunar industrial capacity $P(t)$ as a saturation-limited growth function driven by specific Tier-1 technological inputs:

\begin{equation}
\frac{dP(t)}{dt} = \alpha P(t) \left( 1 - \frac{P(t)}{P_{max}} \right) + \beta \cdot \Phi_{R, Tier1}(t - \tau)
\end{equation}

where $\Phi_{R, Tier1}$ represents the influx of high-complexity "seed" machinery delivered exclusively by rockets. This differential coupling reveals a counter-intuitive \textbf{Environmental Investment Logic}: an aggressive initial deployment of dirty rockets maximizes the term $\beta \cdot \Phi_{R, Tier1}$, rapidly elevating $P(t)$. This accelerated industrialization allows for an earlier transition to high ISRU localization ($k_i \to 1$), thereby drastically reducing the total Earth-to-Moon mass requirement ($\int x_{m,k} dt$) over the project lifecycle.

\subsubsection{Inter-Planetary Liability Exchange}
The optimal solution exhibits a phenomenon we define as \textbf{Inter-Planetary Liability Exchange}. The mission strategically accumulates terrestrial debt $\mathcal{D}_E$ during the "Seeding Phase" (2050-2053) to establish a self-sufficient lunar industrial base. This "investment" yields a net positive return by preventing the much larger cumulative emissions that would result from a prolonged, low-efficiency dependency on Earth-based supply chains. The simulation identifies a \textbf{Green Pareto Frontier}, where the marginal environmental cost of limiting early rocket launches results in a disproportionate increase in total project duration and long-term transport volume.

\begin{figure}[htbp]
\centering
\begin{minipage}{0.48\textwidth}
\centering
\fbox{\parbox[c][5cm]{0.9\linewidth}{\centering \textbf{PLACEHOLDER: Environmental Debt Crossover Plot} \\
\emph{Description:} Dual-line graph with Time (X) vs. Cumulative Emissions (Y). Show "Aggressive Bootstrapping" (Blue) with high initial slope but early plateau, and "Conservative" (Red) with lower initial slope but constant rise. Highlight the "Environmental Crossover Point".}}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
\centering
\fbox{\parbox[c][5cm]{0.9\linewidth}{\centering \textbf{PLACEHOLDER: Green Pareto Flow} \\
\emph{Description:} Stacked area chart showing Earth Debt (Positive Y) decreasing over time and Moon Debt (Negative Y) increasing. Illustrates the "Inter-Planetary Liability Exchange" - shifting burden from Earth atmosphere to Moon surface.}}
\end{minipage}
\caption{Strategic Environmental Dynamics. (Left) The crossover point confirms that high initial investment leads to lower long-term debt. (Right) The exchange of liabilities protects the terrestrial stratosphere at the cost of localized lunar disturbance.}
